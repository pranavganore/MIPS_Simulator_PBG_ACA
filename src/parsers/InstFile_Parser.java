...
public class InstFile_Parser {
	...
	public static void parse(String filePath) throws Exception {
		...
		Program_Manager.instance.InstructionList.put(
				Program_Manager.instance.instructionCount++, inst);
	...
	private static void parseInstLine(String line) throws Exception {
		...
		if (line.contains(":")) {
			...
			line = line.trim();
			Program_Manager.instance.LabelMap.put(loopName.trim(),
					Program_Manager.instance.instructionCount);
		}
		...
		case "BNE":
			...
			break;
		case "HLT":
//			if (firstHaltProcessed == 0) {
				inst = new HLT();
//			}else if (firstHaltProcessed == 1) { //parsing 2nd HLT 
//				inst = new HLT2();
//			}
			
			firstHaltProcessed = 1;
			break;
		case "J":
			operands = getOperands(tokens);
			destinationRegister = operands[0];
			...
			for (int i = 0; i < argListArray.length; i++) {
				...
				if (arg.charAt(0) != 'R' && arg.charAt(0) != 'F') {
					if (arg.charAt(0) < '0' || arg.charAt(0) > '9')
						if (!Program_Manager.instance.LabelMap.containsKey(arg))
